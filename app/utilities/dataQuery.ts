...
export async function filterDistricts(provinceId: number) {
  ...
  const zipcodes: ZipCode[] = zipcodeSnapshot.docs.map(
    (doc) => doc.data() as ZipCode
  );
  const sub_districts: SubDistrictDetail[] = subDistrictsSnapshot.docs.map(
    (doc) => ({
      ...(doc.data() as SubDistrict),
      zipcode: zipcodes.find(
        (zipcode) => zipcode.id == doc.get("zipCodeId")
      ) as ZipCode,
    })
  );
  const districts: DistrictDetail[] = districtsSnapShot.docs.map((doc) => ({
    ...(doc.data() as District),
    sub_districts: sub_districts.filter(
      (sub_district) => sub_district.districtId == doc.get("id")
    ),
  }));
  return districts;
}
