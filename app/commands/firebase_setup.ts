...
export default class FirebaseSetup implements Command {
	...
	async setData(collectionName: string, fileName: string,interfacesName: string) {
		console.log(`Setting ${collectionName}`);
		const model = ( await import(`../models/${interfacesName}`) ).default;
		const file = fs.readFileSync(path.join(path.resolve()) + `/dist/${fileName}`);
		const data: typeof model[] = JSON.parse(file.toString());
		data.forEach(async item => {
			try {
				console.log(`Set ${item.nameEn || item.id}`);
				await setDoc(doc(this.db, collectionName, item.id.toString()), item, {merge: true});	
			} catch (error) {
				console.trace(error);
			}	
		});	
	...
	async excecute() {
		...
		await this.setData('sub_districts', 'subDistricts_new.json', 'sub_district');
