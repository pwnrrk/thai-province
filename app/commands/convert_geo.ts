...
export default class ConvertGeo implements Command {
	excecute() {
		const file = fs.readFileSync(path.join(path.resolve()) + '/dist/geography.json');
		...
		fs.writeFileSync(path.join(path.resolve()) + '/dist/geography_new.json', JSON.stringify(geographs));
	}
}
