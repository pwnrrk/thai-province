...
export default class ConvertSubDistrict implements Command {
  ...
  excecute() {
    const file = fs.readFileSync(
      path.join(path.resolve()) + "/dist/subDistricts.json"
    );
    ...
    const subDistricts: SubDistrict[] = data.map((subDistrict) => {
      const fetched: OldZipCodeFormat = oldZipcodes.find(
        (zip) => parseInt(zip.SUB_DISTRICT_ID) == subDistrict.SUB_DISTRICT_ID
      ) as OldZipCodeFormat;
      const zipcodeId: number = fetched
        ? (newZipcodes.find((zip) => zip.code == fetched.ZIPCODE) as ZipCode).id
        : 0;
      return {
